...
npv_data <- subset(long_results, Metric == "NPV")
bcr_data <- subset(long_results, Metric == "BCR")

colors <- scale_fill_viridis(discrete = TRUE)
npv_color <- viridis(2)[1]
bcr_color <- viridis(2)[2]

npv <- ggplot(npv_data, aes(x = as.factor(DiscountRate), y = Value)) +
  geom_bar(aes(fill = "NPV"), stat = "identity", position = "dodge") +
  scale_y_continuous(labels = scales::dollar, limits = c(min(npv_data$Value), max(npv_data$Value))) +
  scale_fill_manual(values = c("NPV" = npv_color)) +
  labs(title = "Net Present Value (NPV)", x = "Discount Rate", y = NULL, fill = NULL) +
  theme_Publication() +
...
# Plot for BCR
bcr <- ggplot(bcr_data, aes(x = as.factor(DiscountRate), y = Value)) +
  geom_bar(aes(fill = "BCR"), stat = "identity", position = "dodge") +
  geom_point(aes(color = "BCR"), size = 3) +
  geom_line(aes(group = Metric, color = "BCR", linetype = "BCR"), linewidth = 1) +
  scale_y_continuous(labels = scales::percent_format(scale = 1), limits = c(min(bcr_data$Value), max(bcr_data$Value))) +
  scale_fill_manual(values = c("BCR" = bcr_color)) +
  scale_color_manual(values = c("BCR" = bcr_color)) +
  scale_linetype_manual(values = c("BCR" = "solid")) +
  labs(title = "Benefit-Cost Ratio (BCR)", x = "Discount Rate", y = NULL, fill = NULL, color = NULL) +
  theme_Publication() +
  theme(legend.position = "bottom")
